<div>
    @if (auth()->user()->role === 'admin')
        <div class="mt-20 px-5 md:px-20 lg:px-32 grid gap-2 md:grid-cols-2 lg:grid-cols-3 mb-5">
                ...
                <div
                    ...
                    <div class="grid p-4 gap-1 content-start">
                        ...
                        <a href="/detail/{{ $room->id }}"
                            class="bg-green-600 text-white text-center w-full px-3 py-2 mt-2 text-sm  hover:bg-green-700 transition">Detail</a>
                    </div>
                </div>
            @endforeach
            {{-- <div class="w-full grid grid-cols-2 shadow-md overflow-hidden border border-gray-200 h-40 bg-white">
                <div class="relative">
                    <div
                        class="absolute flex items-center text-yellow-600 text-xs gap-1 bg-white px-2 py-1 top-3 left-3 rounded-full shadow">
                    ...
                    <img src="https://fresh.suakaonline.com/wp-content/uploads/2017/04/1490486838098.jpg"
                        alt="Room Image" class="w-full h-full object-cover" />
                </div>

                <div class="grid p-4 gap-1 content-start">
                    <h1 class="text-base font-semibold text-slate-700">Kamar 01B</h1>
                    <p class="text-sm text-gray-600">Kelapa Gading</p>
                    <h2 class="text-lg font-semibold text-slate-700">Rp. 1.500.000</h2>
                    <a href="/detail/on-proccess"
                        class="bg-green-600 text-white text-center w-full px-3 py-2 mt-2 text-sm  hover:bg-green-700 transition">Detail</a>
                </div>
            </div>
            <div class="w-full grid grid-cols-2 shadow-md overflow-hidden border border-gray-200 h-40 bg-white">
            ...
            <div class="w-full grid grid-cols-2 shadow-md overflow-hidden border border-gray-200 h-40 bg-white">
                <div class="relative">
                    <div
                        class="absolute flex items-center text-gray-600 text-xs gap-1 bg-white px-2 py-1 top-3 left-3 rounded-full shadow">
                        <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path fill-rule="evenodd"
                                d="M3 4a1 1 0 0 0-.822 1.57L6.632 12l-4.454 6.43A1 1 0 0 0 3 20h13.153a1 1 0 0 0 .822-.43l4.847-7a1 1 0 0 0 0-1.14l-4.847-7a1 1 0 0 0-.822-.43H3Z"
    ...
    @else
        @if (!$detail)
            <div class="mt-24 text-center font-bold text-green-800">Anda Tidak Terdaftar di JDS Kost</div>
        @else
            <div class="mt-20 mb-10 px-5 md:px-20 lg:px-32 lg:grid lg:grid-cols-3 gap-4">
                <div class='lg:col-span-2 bg-white shadow-lg p-4 relative rounded-md min-h-[60vh] mb-5 lg:mb-0'>
                    <div class="bg-green-600 opacity-80 absolute left-5 right-5 top-5 -bottom-3 -z-10 rounded-md"></div>
                    <div class="flex justify-between">
                        <h1 class="text-lg font-bold text-green-600">Rincian Pembayaran</h1>
                        {{-- <h1 class="text-md font-bold text-green-600">{{auth()->user()->name}}</h1> --}}
                    </div>
                    <hr class="my-2" />
                    @if ($payments->isEmpty())
                    ...
                    @else
                        <table
                            class="w-full border bg-gradient-to-l from-green-500 to-green-700 text-gray-700 rounded-md">
                            <thead class="bg-green-500 text-white">
                                <tr>
                                    <th class="p-2 border border-green-600">No</th>
                                    <th class="p-2 border border-green-600">No Invoice</th>
                                    <th class="p-2 border border-green-600">Tanggal</th>
                            ...
                            <tbody>
                                @foreach ($payments as $idx => $payment)
                                    <tr class="bg-white hover:bg-green-50">
                                        <td class="p-2 border border-green-600 text-center">{{ $idx + 1 }}</td>
                ...
                <div class='lg:col-span-1 bg-white shadow-lg p-4 relative rounded-md min-h-[80vh]'>
                    <div class="bg-green-600 opacity-80 absolute left-5 right-5 top-5 -bottom-3 -z-10 rounded-md"></div>
                    <div class="border border-green-500 bg-green-50 rounded-md p-2 mb-3">
                        <div class="flex justify-between items-center">
                            <h1 class="text-lg font-bold text-green-600">Tagihan</h1>
                            <p class="text-xs font-semibold italic text-gray-600">{{formatRupiah($detail->price)}}/bulan</p>
                        </div>
                        <hr class="my-2" />
                        @if (
                        ...
                        @else
                            <div>
                                <svg class="w-6 h-6 text-green-700 opacity-50 mb-0 text-center mx-auto"
                                ...
                                <p class="mb-2 text-center font-semibold text-xs text-green-700 opacity-50">Tidak Ada
                                    Tagihan
                                </p>
                            </div>
                        @endif
